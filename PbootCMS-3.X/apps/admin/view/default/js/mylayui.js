layui.use(['element', 'upload', 'laydate', 'form'], function() {
	...
	form.on('submit(login-submit)', function(data) {
		...
		$.ajax({
			...
			error: function(xhr, status, error) {
				...
				$('#note').html(
					'登录请求发生错误，您可按照如下方式排查：<br>1、试着删除根目录下runtime目录，刷新页面重试;<br>2、检查系统会话文件存储目录是否具有写入权限；<br>3、检查服务器环境pathinfo及伪静态规则配置；<br>4、如果还不行那就到交流群出钱请人处理吧！'
				);
			}
		});
		return false;
	...
	var uploadInst = upload.render({
		...
		before: function(obj) {
			...
			console.log(uploadInst.config.url)
			...
			if (res.code == 1) {
				$('#' + des).val(res.data[0]);
				$('#' + des + '_box').html("<dl><dt><img src='" + sitedir + res.data[0] + "' data-url='" + res.data[0] +
					"' ></dt><dd>删除</dd></dl>");
				layer.msg('上传成功！');
			} else {
	...
	var uploadsInst = upload.render({
		...
		before: function(obj) {
			...
			if (res.code == 1) {
				...
				html += "<dl><dt><img src='" + sitedir + res.data[0] + "' data-url='" + res.data[0] +
					"'></dt><dd>删除</dd></dl>";
			} else {
